...
namespace InfinitusApp.Core.Extensions
{
    public static class ISO3166
    {
        ...
        {
            return GetCollection().FirstOrDefault(p => p.Alpha3.ToUpper().Equals(alpha3.ToUpper()));
        }
        ...
        {
            return GetCollection().FirstOrDefault(p => p.Alpha2.ToUpper().Equals(alpha2.ToUpper()));
        }

        /// <summary>
        /// Obtain ISO3166-1 Country based on its numeric code.
        /// </summary>
        /// <param name="numericCode"></param>
        /// <returns></returns>
        public static ISO3166Country FromNumericCode(int numericCode)
        {
            return GetCollection().FirstOrDefault(p => p.NumericCode.Equals(numericCode));
        }

        public static IEnumerable<ISO3166Presentation> GetPresentation(ISO3166Presentation.PresentationType typePresentation)
        {
            switch (typePresentation)
            {
                case ISO3166Presentation.PresentationType.Alpha2:
                    return GetCollection().Select(x => new ISO3166Presentation
                    {
                        Text = x?.Name + " +" + x?.DialCodes?.FirstOrDefault(),
                        Value = x?.DialCodes?.FirstOrDefault()
                    }).OrderBy(x => x.Text);

                case ISO3166Presentation.PresentationType.Alpha3:
                    return GetCollection().Select(x => new ISO3166Presentation
                    {
                        Text = x?.Alpha3 + " +" + x?.DialCodes?.FirstOrDefault(),
                        Value = x?.DialCodes?.FirstOrDefault()
                    }).OrderBy(x => x.Text);

                case ISO3166Presentation.PresentationType.Name:
                default:
                    return GetCollection().Select(x => new ISO3166Presentation
                    {
                        Text = x?.Name + " +" + x?.DialCodes?.FirstOrDefault(),
        ...
        public static IEnumerable<ISO3166Country> GetCollection()
        {
            ...
            return new[] {
             ...
             }.Where(x => x?.DialCodes?.FirstOrDefault() != null);
        }
    ...
    public class ISO3166Presentation
    {
